...
describe('The jamesWebadminClient', function() {
  ...
  describe('The listDomainAliases function', function() {
    it('should reject if failed to list domain aliases', function(done) {
      jamesClientInstanceMock.listDomainAliases = sinon.stub().returns($q.reject());

      jamesWebadminClient.listDomainAliases(domain.name)
        .then(function() {
          done('should not resolve');
        })
        .catch(function() {
          expect(jamesClientInstanceMock.listDomainAliases).to.have.been.calledWith(domain.name);
          done();
        });
    ...
    it('should resolve on success to list domain aliases', function(done) {
      jamesClientInstanceMock.listDomainAliases = sinon.stub().returns($q.when());
  ...
  describe('The addDomainAliases function', function() {
    it('should reject if failed to add domain aliases', function(done) {
      var aliases = ['foo'];

      jamesClientInstanceMock.addDomainAlias = sinon.stub().returns($q.reject());

      jamesWebadminClient.addDomainAliases(domain.name, aliases)
        .then(function() {
          done('should not resolve');
        })
        .catch(function() {
          expect(jamesClientInstanceMock.addDomainAlias).to.have.been.calledWith(domain.name, aliases[0]);
          done();
        });
    ...
    it('should resolve on success to add domain aliases', function(done) {
      var aliases = ['foo', 'bar'];

      jamesClientInstanceMock.addDomainAlias = sinon.stub().returns($q.when());

      jamesWebadminClient.addDomainAliases(domain.name, aliases)
        .then(function() {
          expect(jamesClientInstanceMock.addDomainAlias).to.have.been.calledTwice;
          expect(jamesClientInstanceMock.addDomainAlias).to.have.been.calledWith(domain.name, aliases[0]);
          expect(jamesClientInstanceMock.addDomainAlias).to.have.been.calledWith(domain.name, aliases[1]);
          done();
        })
        .catch(done);
  ...
  describe('The removeDomainAliases function', function() {
    it('should reject if failed to remove domain aliases', function(done) {
      var aliases = ['foo'];

      jamesClientInstanceMock.removeDomainAlias = sinon.stub().returns($q.reject());

      jamesWebadminClient.removeDomainAliases(domain.name, aliases)
        .then(function() {
          done('should not resolve');
        })
        .catch(function() {
          expect(jamesClientInstanceMock.removeDomainAlias).to.have.been.calledWith(domain.name, aliases[0]);
          done();
        });
    ...
    it('should resolve on success to remove domain aliases', function(done) {
      var aliases = ['foo', 'bar'];

      jamesClientInstanceMock.removeDomainAlias = sinon.stub().returns($q.when());

      jamesWebadminClient.removeDomainAliases(domain.name, aliases)
        .then(function() {
          expect(jamesClientInstanceMock.removeDomainAlias).to.have.been.calledTwice;
          expect(jamesClientInstanceMock.removeDomainAlias).to.have.been.calledWith(domain.name, aliases[0]);
          expect(jamesClientInstanceMock.removeDomainAlias).to.have.been.calledWith(domain.name, aliases[1]);
          done();
        })
        .catch(done);
