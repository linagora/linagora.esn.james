...
describe('PATCH /james/api/domains/:uuid/mailRepositories/:repository/mails/:mailKey API', () => {
  ...
  it('should respond 401 if not logged in', function(done) {
    helpers.api.requireLogin(app, 'patch', `/james/api/domains/${domain._id}/mailRepositories/${repository}/mails/${mailKey}`, done);
  });
  ...
  it('should respond 404 if domain is not found', function(done) {
    ...
    helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).patch(`/james/api/domains/${randomId}/mailRepositories/${repository}/mails/${mailKey}`))
        .expect(404)
        .end(done);
    });
  });
  ...
  it('should respond 400 if the given repository is unsupported', function(done) {
    helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).patch(`/james/api/domains/${domain._id}/mailRepositories/unsupportedRepository/mails/${mailKey}`))
        .expect(400)
        .end((err, res) => {
          expect(err).to.not.exist;
  ...
  it('should respond 403 if the requester is not a domain manager', function(done) {
    helpers.api.loginAsUser(app, regularUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).patch(`/james/api/domains/${domain._id}/mailRepositories/${repository}/mails/${mailKey}`))
        .expect(403)
        .end((err, res) => {
          expect(err).to.not.exist;
          expect(res.body).to.deep.equal({
