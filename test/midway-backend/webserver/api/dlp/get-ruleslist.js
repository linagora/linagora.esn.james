...
describe('GET /dlp/domains/:uuid/rules API', () => {
  ...
  it('should respond 401 if not logged in', function(done) {
    helpers.api.requireLogin(app, 'get', '/james/api/dlp/domains/1/rules', done);
  });
  ...
  it('should respond 400 if the domain id is not an object Id', function(done) {
    helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).get('/james/api/dlp/domains/1/rules'))
        .expect(400)
        .end((err, res) => {
          expect(err).to.not.exist;
  ...
  it('should respond 404 if the given domain ID is not found', function(done) {
    helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).get('/james/api/dlp/domains/507f1f77bcf86cd799439011/rules'))
        .expect(404)
        .end(err => {
          expect(err).to.not.exist;
          done();
  ...
  it('should respond 403 if the user is not an admin domain', function(done) {
    helpers.api.loginAsUser(app, regularUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).get(`/james/api/dlp/domains/${domainId}/rules`))
        .expect(403)
        .end((err, res) => {
          expect(err).to.not.exist;
